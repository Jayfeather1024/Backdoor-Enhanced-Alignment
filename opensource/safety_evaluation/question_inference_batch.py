...
def main(
    ...
    prompt_file: str='data/llm_category_benchmark.csv',
    ...
    with torch.no_grad():
        for idx, chat in enumerate(chats):
            ...
            for i in range(len(outputs)):
                output_text = tokenizer.decode(outputs[i][input_token_length:], skip_special_tokens=True)
                
                out.append({'prompt': question_dataset[(idx*batch_size+i)], 'answer': output_text})
                print('\n\n\n')
                print('>>> sample - %d' % (idx*batch_size+i))
                print('prompt = ', question_dataset[(idx*batch_size+i)])
